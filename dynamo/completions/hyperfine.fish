...
complete -c hyperfine -s u -l time-unit -d 'Set the time unit to be used. Possible values: millisecond, second.' -r -f -a "{millisecond	,second	}"
complete -c hyperfine -l export-asciidoc -d 'Export the timing summary statistics as an AsciiDoc table to the given FILE.' -r
complete -c hyperfine -l export-csv -d 'Export the timing summary statistics as CSV to the given FILE. If you need the timing results for each individual run, use the JSON export format.' -r
...
complete -c hyperfine -l output -d 'Control where the output of the benchmark is redirected. <WHERE> can be:
